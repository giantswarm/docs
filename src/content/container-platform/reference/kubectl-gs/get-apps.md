...
menu:
  principal:
    identifier: container-platform-reference-kubectl-gs-get-apps
...
**Deprecated:** This command lists the legacy `App` custom resource, which is being phased out in favor of Flux HelmRelease. For new deployments, use `flux get helmreleases` (or `kubectl get helmreleases`). See [App management]({{< relref "/container-platform/overview/fleet-management/app-management" >}}) for the conceptual overview.

Like with all `get` commands in `kubectl`, this command can be used to get details on one item, an [App]({{< relref "/container-platform/reference/platform-api/crd/apps.application.giantswarm.io.md" >}})
...
The following example command would print the main resource for app `coredns`. It would return the [App]({{< relref "/container-platform/reference/platform-api/crd/apps.application.giantswarm.io.md" >}}) resource.
