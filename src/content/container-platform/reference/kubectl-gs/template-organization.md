...
menu:
  principal:
    identifier: container-platform-reference-kubectl-gs-template-organization
...
---
The `template organization` command creates an [organization]({{< relref "/container-platform/overview/fleet-management/multi-tenancy" >}})
manifest which can be applied to a management cluster, e. g. via `kubectl apply`.
The manifest will define an [`Organization`]({{< relref "/container-platform/reference/platform-api/crd/organizations.security.giantswarm.io.md" >}})
