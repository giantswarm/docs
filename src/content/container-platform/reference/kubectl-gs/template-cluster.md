...
menu:
  principal:
    identifier: container-platform-reference-kubectl-gs-template-cluster
...
- [`App`]({{< relref "/container-platform/reference/platform-api/crd/apps.application.giantswarm.io.md" >}}) - describes the Giant Swarm App which defines the helm release which in turn creates the actual cluster resources. The resource name is identical with the cluster name.
- `ConfigMap` - describes the configuration for the above cluster chart. Please see [Creating a workload cluster]({{< relref "/container-platform/getting-started/provision-your-first-workload-cluster/" >}}) for which cluster chart is used, depending on the cloud provider. The resource name is the cluster name plus the `-userconfig` suffix.
