...
menu:
  principal:
    ...
    identifier: container-platform-overview-security
...
- **External Secrets Operator**: Reads secrets from an external secret manager, such as AWS KMS, Azure Key Vault, or Hashicorp Vault, and delivers them as Kubernetes secrets, so credentials never have to be committed to source control. See [External Secrets Operator]({{< relref "/container-platform/overview/security/external-secrets-operator/" >}}).

Learn how to start with Security on Giant Swarm by visiting our [getting started security page]({{< relref "/container-platform/overview/security/platform-security/" >}}).
