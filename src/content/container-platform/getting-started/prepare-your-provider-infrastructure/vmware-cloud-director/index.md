...
menu:
  principal:
    identifier: container-platform-getting-started-prepare-your-provider-infrastructure-vmware-cloud-director
...
aliases:
  - /getting-started/prepare-your-provider-infrastructure/vmware-cloud-director/
...
- Access to the Internet is required for some of our automation to pull artifacts from the registry or get new certificates from Let's Encrypt. It's possible to configure an egress HTTP proxy to control the traffic. You can check the list of domains that need to be accessible [here]({{< relref "/container-platform/overview/security/domain-allowlist/#on-premise-installations" >}}).
...
After all requirements are met, you can [create your first cluster following this guide]({{< relref "/container-platform/getting-started/provision-your-first-workload-cluster/" >}}).

## Next step

If you are running these steps for the first time and still don't have a management cluster, Giant Swarm will provide it in the next few days. For sharing any secret with us please read [this article]({{< relref "/container-platform/overview/security/sharing-secrets" >}}) first.

If you already have a management cluster, you can proceed with the next step and learn how to [access to platform API]({{< relref "/container-platform/getting-started/access-to-platform-api" >}}).
