...
menu:
  principal:
    ...
    identifier: container-platform-getting-started-access-to-platform-api
aliases:
  - /getting-started/access-to-platform-api/
...
2. Use the `kubectl` command-line tool with our [custom plugin]({{< relref "/container-platform/reference/kubectl-gs" >}})
3. Use the [developer portal]({{< relref "/container-platform/overview/developer-portal" >}})

This guide focuses on the second option, using the `kubectl` command-line tool. However, you can find more information about the other options in the [tutorials]({{< relref "/container-platform/tutorials" >}}).
...
When logging in, you must authenticate in your browser using the configured identity provider (for example, by signing into an `Active Directory` or `GitHub` user account). To learn more about the different authentication methods, see the [authentication page]({{< relref "/container-platform/overview/architecture/authentication" >}}).
...
Organizations are a way to separate and isolate clusters, apps, etc., between different teams or environments. More information can be found in [Multi-tenancy]({{< relref "/container-platform/overview/fleet-management/multi-tenancy" >}}).
...
Now that you have access to the platform API and know a bit more about it, you are ready to [create a workload cluster]({{< relref "/container-platform/getting-started/provision-your-first-workload-cluster/" >}}).
