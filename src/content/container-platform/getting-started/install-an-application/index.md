...
menu:
  principal:
    ...
    identifier: container-platform-getting-started-install-an-application
owner:
...
aliases:
  - /getting-started/install-an-application/
---
...
For a deeper reference covering every Flux flag and option, see [Deploying an application via a Flux HelmRelease]({{< relref "/container-platform/tutorials/fleet-management/app-platform/deploy-app-helmrelease" >}}).

## Requirements

You need:

- A running workload cluster. If you don't have one, follow [Provision your first workload cluster]({{< relref "/container-platform/getting-started/provision-your-first-workload-cluster" >}}).
- Gateway API and Envoy Gateway installed in that cluster. The quickest way is the Gateway API bundle. See [Installing Gateway API with Envoy Gateway]({{< relref "/container-platform/tutorials/connectivity/gateway-api/installation" >}}). Once installed, the bundle creates a default Gateway named `giantswarm-default` that your routes can attach to.
- The [Flux CLI](https://fluxcd.io/flux/cmd/) installed locally.
- Access to the management cluster (see [kubectl gs login]({{< relref "/container-platform/reference/kubectl-gs/login" >}})).
...
You'll see the apps that ship with the cluster: the default-apps bundle, observability tooling, and other managed components. Confirm the Gateway API bundle is among them. If it isn't, install it first using the [Gateway API installation guide]({{< relref "/container-platform/tutorials/connectivity/gateway-api/installation" >}}).
...
You should get back `HTTP/1.1 200 OK`. To enable HTTPS, add the `hello` subdomain to your Gateway's HTTPS listener and configure cert-manager for Gateway API. See [Installing Gateway API with Envoy Gateway]({{< relref "/container-platform/tutorials/connectivity/gateway-api/installation" >}}).
...
After learning how to deploy an application in the workload cluster, dive into [how networking works in your cluster and how to configure security hardening of your application]({{< relref "/container-platform/getting-started/understand-connectivity" >}}).
