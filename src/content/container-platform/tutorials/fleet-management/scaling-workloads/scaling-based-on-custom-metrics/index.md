...
menu:
  principal:
    parent: container-platform-tutorials-fleet-management-scaling-workloads
    identifier: container-platform-tutorials-fleet-management-scaling-workloads-scaling-based-on-custom-metrics
...
aliases:
  - /tutorials/fleet-management/scaling-workloads/scaling-based-on-custom-metrics/
---
...
- A Giant Swarm workload cluster with GPU-enabled nodes (see our [GPU workloads guide]({{< relref "/container-platform/tutorials/fleet-management/cluster-management/gpu" >}}))
- `kubectl` configured to access your workload cluster
- Access to the platform API (see [accessing the platform API]({{< relref "/container-platform/getting-started/access-to-platform-api" >}}))
...
First, confirm that GPU metrics are being collected and available in your observability platform. The DCGM exporter should already be configured as described in the [GPU monitoring section]({{< relref "/container-platform/tutorials/fleet-management/cluster-management/gpu#monitoring" >}}).
...
You can also access the general [Grafana dashboard]({{< relref "/observability-platform/overview/dashboard-management" >}}) to visualize the GPU consumption nicer.

## Next steps

Now that you have HPA configured with custom KEDA and Mimir, consider these additional optimizations:

- **[Configure cluster autoscaling]({{< relref "/container-platform/tutorials/fleet-management/cluster-management/aws-cluster-scaling" >}})** to automatically provision GPU nodes when needed
- **[Create custom dashboards]({{< relref "/observability-platform/overview/dashboard-management/dashboard-creation" >}})** to visualize your GPU scaling patterns
- **[Implement resource quotas]({{< relref "/container-platform/tutorials/fleet-management/cluster-management/node-pools#resource-management" >}})** to control GPU allocation in multi-tenant environments
