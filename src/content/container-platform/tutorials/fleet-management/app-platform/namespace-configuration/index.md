...
weight: 60
aliases:
  - /tutorials/fleet-management/app-platform/namespace-configuration/
...
menu:
  principal:
    ...
    identifier: container-platform-tutorials-fleet-management-app-platform-namespace-configuration
owner:
...
The [`App` custom resource]({{< relref "/container-platform/reference/platform-api/crd/apps.application.giantswarm.io.md" >}}) allows you to configure the target namespace via the spec.
...
The [validation logic]({{< relref "/container-platform/tutorials/fleet-management/app-platform/defaulting-validation" >}}) in app admission controller checks whether multiple `App` resources are updating the same namespace with different values. The validation webhook will prevent the conflicting value from being added.
