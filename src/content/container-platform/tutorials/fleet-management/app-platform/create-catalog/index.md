...
weight: 50
aliases:
  - /tutorials/fleet-management/app-platform/create-catalog/
...
menu:
  principal:
    ...
    identifier: container-platform-tutorials-fleet-management-app-platform-create-catalog
owner:
...
Every app catalog has its own [`Catalog`]({{< relref "/container-platform/reference/platform-api/crd/catalogs.application.giantswarm.io.md" >}}) custom resource defined in the platform API.
...
These metadata files and the `helm` `index.yaml` are used to generate app metadata that's stored in the platform  as an [`AppCatalogEntry`]({{< relref "/container-platform/reference/platform-api/crd/appcatalogentries.application.giantswarm.io.md" >}}) resource.
...
To register the catalog with app platform you need to create a `Catalog` resource in the platform. This can be done using [`kubectl gs template catalog`]({{< relref "/container-platform/reference/kubectl-gs/template-catalog" >}}).
...
You should create the `Catalog` resource in the [organization]({{< relref "/container-platform/overview/fleet-management/multi-tenancy/" >}}) namespace where the apps will be used. Or if it needs to be used in multiple organizations it can be created in the `default` namespace.
...
If its value is `public` it will appear in the [developer portal]({{< relref "/container-platform/overview/developer-portal" >}}). Otherwise it will be hidden.

## Install an app from the catalog

If the `Catalog` resource isn't in the `default` namespace you need to set the catalog namespace to the organization namespace where it's stored. This can be done using [`kubectl gs template app`]({{< relref "/container-platform/reference/kubectl-gs/template-app" >}}).
