...
menu:
  principal:
    identifier: container-platform-tutorials-continuous-deployment-helm-releases-multiple-releases
...
aliases:
  - /tutorials/continuous-deployment/helm-releases/multiple-releases/
---
...
2. **Kustomize over multiple `HelmReleases`.** Keep each component as a separate `HelmRelease` and `OCIRepository`, then use a Kustomize base to apply shared labels, a common namespace, and version pins. Closest to the [Giant Swarm App Sets pattern]({{< relref "/container-platform/tutorials/continuous-deployment/apps/app-sets" >}}).
