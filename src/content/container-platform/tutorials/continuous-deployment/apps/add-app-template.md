...
menu:
  principal:
    identifier: container-platform-tutorials-continuous-deployment-apps-add-app-template
...
Now, navigate to the newly created directory and use [the kubectl-gs plugin](https://github.com/giantswarm/kubectl-gs) to generate the [`App` resource]({{< relref "/container-platform/reference/kubectl-gs/template-app" >}}):
...
Reference [the app configuration]({{< relref "/container-platform/tutorials/fleet-management/app-platform/app-configuration/" >}}) for more details about how to create the respective `ConfigMaps` or secrets.
...
**Warning**: `Kustomize` can't be used for secrets because they must be encrypted before committing. Refer to our [adding an app]({{< relref "/container-platform/tutorials/continuous-deployment/apps/add-appcr" >}}) docs to check how to do it.
...
At this point, you should have an app template ready. Now, you can [add a new app to a workload cluster using the template]({{< relref "/container-platform/tutorials/continuous-deployment/apps/add-appcr" >}}).
