...
menu:
  principal:
    parent: container-platform-tutorials-continuous-deployment
    identifier: container-platform-tutorials-continuous-deployment-manage-workload-clusters
user_questions:
...
aliases:
  - /tutorials/continuous-deployment/manage-workload-clusters/
---
...
**Note**: To learn more about managing roles, please read the [introduction to platform API]({{< relref "/container-platform/getting-started/access-to-platform-api" >}}).
...
Now, the `GitRepository` resource is created and exported in the `management-clusters` folder. Next, you apply the resource towards the [management cluster API]({{< relref "/container-platform/getting-started/access-to-platform-api" >}}) where the resources will run:
...
The next step is to configure the keys used by `Flux` in the management cluster to decipher secrets kept in the repository. We recommend maintaining secrets encrypted in the repository alongside your applications. But if your company policy doesn't allow it, you can use [External Secrets Operator]({{< relref "/container-platform/overview/security/external-secrets-operator/" >}}) to use different sources.
...
The `organization` resource is the main entity to enable [multi-tenancy]({{< relref "/container-platform/overview/fleet-management/multi-tenancy" >}}) in management clusters.
...
Read the [base creation tutorial]({{< relref "/container-platform/tutorials/continuous-deployment/bases" >}}) if you want to create your own base. Otherwise, you can use the bases provided by Giant Swarm.
...
Keep learning GitOps [reading how to use different configuration by environment]({{< relref "/container-platform/tutorials/continuous-deployment/environments/" >}}).
