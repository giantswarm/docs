...
menu:
  principal:
    ...
    identifier: container-platform-tutorials-continuous-deployment-bases
user_questions:
...
aliases:
  - /tutorials/continuous-deployment/bases/
---

In Giant Swarm the interface to define a workload cluster is built on top of `Helm` and [the app platform]({{< relref "/container-platform/overview/fleet-management/app-management/" >}}). The application custom resource contains the specification and configuration of the cluster in this format:
...
The current possible values for the providers can be checked in the [command reference]({{< relref "/container-platform/reference/kubectl-gs/gitops/add-base" >}}).
...
2. Use the [kubectl gs template cluster]({{< relref "/container-platform/reference/kubectl-gs/template-cluster" >}}) to template cluster resources, see an example for the `capa` provider below. Use arbitrary values for the mandatory fields, we will configure them later in our process:
...
Now learn [how to create workload clusters]({{< relref "/container-platform/tutorials/continuous-deployment/manage-workload-clusters/" >}}) using these bases.
