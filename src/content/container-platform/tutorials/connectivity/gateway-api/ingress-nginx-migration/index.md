...
menu:
  principal:
    parent: container-platform-tutorials-connectivity-gateway-api
    identifier: container-platform-tutorials-connectivity-gateway-api-ingress-nginx-migration
owner:
...
aliases:
  - /tutorials/connectivity/gateway-api/ingress-nginx-migration/
---
...
Before migrating, it's helpful to understand how Ingress controller ingress-nginx works in Giant Swarm clusters. For detailed information, see the [exposing workloads guide]({{< relref "/container-platform/tutorials/connectivity/ingress/exposing-workloads" >}}).
...
**Note:** The Gateway API Bundle currently ships as an [`App` custom resource]({{< relref "/container-platform/overview/fleet-management/app-management" >}}), which is being phased out in favor of Flux HelmRelease. The bundle itself doesn't yet have a HelmRelease equivalent because it deploys nested App resources internally.
...
- [Gateway API installation guide]({{< relref "/container-platform/tutorials/connectivity/gateway-api/installation" >}})
- [Gateway API usage examples]({{< relref "/container-platform/tutorials/connectivity/gateway-api/usage" >}})
