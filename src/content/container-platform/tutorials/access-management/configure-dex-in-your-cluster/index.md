...
menu:
  principal:
    ...
    identifier: container-platform-tutorials-access-management-configure-dex-in-your-cluster
user_questions:
...
aliases:
  - /tutorials/access-management/configure-dex-in-your-cluster/
---
...
Specifying `teams` enables Dex's [group filtering]({{< relref "/container-platform/tutorials/access-management/configure-dex-in-your-cluster#dex-group-filtering" >}}).
...
Specifying `groups` and `useGroupsAsWhitelist` enables Dex's [group filtering]({{< relref "/container-platform/tutorials/access-management/configure-dex-in-your-cluster#dex-group-filtering" >}}).
...
**Note**: The `App` custom resource shown below is being phased out in favor of Flux HelmRelease. For new deployments, see [Deploying an application via a Flux HelmRelease]({{< relref "/container-platform/tutorials/fleet-management/app-platform/deploy-app-helmrelease" >}}).
...
**Warning**: It's assumed that you have an [ingress controller and cert-manager]({{< relref "/container-platform/getting-started/install-an-application" >}}) running in your cluster, so Dex is available for the callback request made by your identity provider securely. Both of these apps are offered in our managed app catalog. If you supply custom certificates when deploying `Dex`, you can skip cert-manager installation.
...
To get an overview on the authentication success and error rates of your `Dex` instances, our product offers a Grafana dashboard named "Dex" as part of our [observability setup]({{< relref "/observability-platform/overview/data-management/data-exploration/" >}}).

## Logging into your workload cluster via `kubectl gs` with `Athena`

Once `Dex` is set up in your workload cluster, you can enable access via OIDC through our `kubectl` plugin [kubectl gs]({{< relref "/container-platform/reference/kubectl-gs" >}}).
...
For easy integration with `kubectl gs` you can install [Athena](https://github.com/giantswarm/athena) on your workload cluster via the [app platform]({{< relref "/container-platform/getting-started/install-an-application" >}}).
...
Learn how to [use `ServiceAccount` to authenticate against platform API]({{< relref "/container-platform/tutorials/access-management/authentication/automation" >}}).
