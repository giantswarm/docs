...
A custom OAuth server showing `Auth Required` is normal until a user logs in. If it shows `Failed`, the endpoint is unreachable—check the URL, network policy, and that the server actually speaks the transport you declared. The [troubleshooting guide]({{< relref "/tutorials/agent-platform/troubleshooting" >}}) covers the common failure modes.

## Related

- [Manage MCP servers]({{< relref "/tutorials/agent-platform/managing-mcp-servers" >}}): the full `MCPServer` field set.
- [Map RBAC and SSO]({{< relref "/tutorials/agent-platform/access-control" >}}): when the custom server shares your identity provider.
