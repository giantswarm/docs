...
menu:
  principal:
    ...
    identifier: tutorials-agent-platform-oauth-setup
owner:
...
This guide assumes Muster is already [deployed]({{< relref "/tutorials/agent-platform/self-hosting/deploy-muster" >}}). For the security model behind it, see the [security overview]({{< relref "/overview/agent-platform/security" >}}).
...
How Muster reuses or exchanges the user's token per downstream server is set on each [`MCPServer`]({{< relref "/tutorials/agent-platform/managing-mcp-servers" >}}#configure-authentication) resource, not here.
...
- [Deploy Muster]({{< relref "/tutorials/agent-platform/self-hosting/deploy-muster" >}}): install the charts this guide configures.
- [Map RBAC and SSO]({{< relref "/tutorials/agent-platform/access-control" >}}): connect identity-provider groups to cluster permissions.
- [Multi-cluster token exchange]({{< relref "/tutorials/agent-platform/self-hosting/multi-mc-token-exchange" >}}): cross-cluster single sign-on with RFC 8693.
- [Security model]({{< relref "/overview/agent-platform/security" >}}): per-user visibility and token handling.
