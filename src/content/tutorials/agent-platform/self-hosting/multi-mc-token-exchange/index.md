...
menu:
  principal:
    parent: tutorials-agent-platform-self-hosting
    identifier: tutorials-agent-platform-multi-mc-token-exchange
owner:
...
It builds on [multi-cluster access]({{< relref "/tutorials/agent-platform/multi-cluster-access" >}}), which covers the `MCPServer` family wiring. This page is about the operational setup: secrets, identity-provider trust, and RBAC.
...
Reference the secret from each remote server's `auth.tokenExchange` block. The full family wiring lives in [multi-cluster access]({{< relref "/tutorials/agent-platform/multi-cluster-access" >}}#cross-cluster-single-sign-on-with-token-exchange). The exchange-specific fields are:
...
- **Large tokens overflow ingress buffers.** Tokens that carry many group claims can exceed the default nginx header buffer on the `mcp-kubernetes` ingress. Raise `large_client_header_buffers`, as the [OAuth setup]({{< relref "/tutorials/agent-platform/self-hosting/oauth-setup" >}}) guide describes.
- **A remote cluster can fail on its own.** A single cluster reporting `Disconnected` doesn't take the fleet down. Other clusters keep working while you fix the one. See [troubleshooting]({{< relref "/tutorials/agent-platform/troubleshooting" >}}#a-cluster-shows-as-disconnected).

## Related

- [Multi-cluster access]({{< relref "/tutorials/agent-platform/multi-cluster-access" >}}): the `MCPServer` family and instance-selection contract.
- [Deploy Muster]({{< relref "/tutorials/agent-platform/self-hosting/deploy-muster" >}}): install the central aggregator.
- [Set up OAuth]({{< relref "/tutorials/agent-platform/self-hosting/oauth-setup" >}}): the central single sign-on this exchange extends.
- [Map RBAC and SSO]({{< relref "/tutorials/agent-platform/access-control" >}}): how identity maps to cluster permissions.
