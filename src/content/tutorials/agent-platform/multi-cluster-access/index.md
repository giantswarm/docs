...
menu:
  principal:
    ...
    identifier: tutorials-agent-platform-multi-cluster-access
owner:
...
Register each cluster's `mcp-kubernetes` as a remote [`MCPServer`]({{< relref "/tutorials/agent-platform/managing-mcp-servers" >}}) and put them all in one **family** so their tools appear once, with an argument that selects the target cluster:
...
Where a remote server trusts the **same** issuer as the central Muster, you don't need exchange at all—set `forwardToken: true` instead and Muster reuses the user's token. See [RBAC and SSO]({{< relref "/tutorials/agent-platform/access-control" >}}) for the difference.
...
- **Large tokens overflow default ingress buffers.** Enterprise tokens carrying many group claims can exceed the default nginx header buffer. The `mcp-kubernetes` ingress needs a raised `large_client_header_buffers` to accept them. See [RBAC and SSO]({{< relref "/tutorials/agent-platform/access-control" >}}#large-tokens) for the symptom and fix.

## Related

- [Manage MCP servers]({{< relref "/tutorials/agent-platform/managing-mcp-servers" >}}): the `family` and `auth` fields in full.
- [Map RBAC and SSO]({{< relref "/tutorials/agent-platform/access-control" >}}): mapping identity to cluster permissions.
- [Architecture]({{< relref "/overview/agent-platform/architecture" >}}): how fleet-wide aggregation fits together.
