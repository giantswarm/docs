...
__Warning:__ The ServiceMonitor needs the label `observability.giantswarm.io/tenant: <YOUR-TENANT-NAME>` for the metrics agent to discover it and start collecting metrics. Also, you need to make sure that the tenant you're setting with the label does actually exist in one of the [Grafana organization CRs](https://docs.giantswarm.io/overview/observability/configuration/multi-tenancy/creating-grafana-organization/). Mimir rejects any metrics that target a tenant not referenced by at least one Grafana Organisation!
