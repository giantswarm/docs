...
__Warning:__ The ServiceMonitor needs to be labeled with `observability.giantswarm.io/tenant: <YOUR-TENANT-NAME>` for the metrics agent to be able to discover it and start collecting metrics. Also, you need to make sure that the tenant you're setting with the label does actually exist in one of the [Grafana organization CRs]({{< relref "/overview/observability/configuration/multi-tenancy/creating-grafana-organizations" >}}). ⚠️ __Important__: Any metrics that are sent to a tenant that is not referenced by at least one [Grafana Organization]({{< relref "/overview/observability/configuration/multi-tenancy/creating-grafana-organizations" >}}) is not ingested by Mimir! Learn more about our multi-tenancy in [Multi-tenancy in the observability platform]({{< relref "/overview/observability/configuration/multi-tenancy/concepts" >}})
