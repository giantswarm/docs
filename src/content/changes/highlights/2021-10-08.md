...
Users of `gsctl` will notice deprecation warnings in a growing number of commands. These will inform you on which `kubectl gs` to check out as a replacement. If you are using `gsctl` in automation, these warnings are printed to STDERR and as we hope should not affect your processing of any STDOUT output.
