...
[EFK v0.5.4](https://docs.giantswarm.io/changes/managed-apps/efk-stack-app/v0.5.4/) adds a option to clean stale lock files in an init container. In some cases lock files are not cleaned up automatically when NFS is used for storage. In those cases Elasticsearch can't start.
