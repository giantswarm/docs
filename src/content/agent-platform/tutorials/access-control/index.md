...
menu:
  principal:
    ...
    identifier: agent-platform-tutorials-access-control
owner:
...
aliases:
  - /tutorials/agent-platform/access-control/
  - /tutorials/ai-agents/access-control/
---
...
Because Muster keys visibility on identity, each user only sees tools from servers they've personally authenticated with. This is **per-user tool visibility**: a cluster a user can't reach simply doesn't appear in their tool list. See the [security model]({{< relref "/agent-platform/overview/security" >}}) for the mechanics.
...
- **Token exchange** (`auth.tokenExchange`): for remote clusters with their own identity provider, exchange the central token for one the remote provider issues. See [multi-cluster access]({{< relref "/agent-platform/tutorials/multi-cluster-access" >}}#cross-cluster-single-sign-on-with-token-exchange).
...
- [Security model]({{< relref "/agent-platform/overview/security" >}}): per-user visibility, token forwarding, and exchange.
- [Multi-cluster access]({{< relref "/agent-platform/tutorials/multi-cluster-access" >}}): cross-cluster single sign-on.
- [Troubleshooting]({{< relref "/agent-platform/tutorials/troubleshooting" >}}): authentication loops and missing tools.
