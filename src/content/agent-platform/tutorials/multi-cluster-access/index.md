...
menu:
  principal:
    ...
    identifier: agent-platform-tutorials-multi-cluster-access
owner:
...
aliases:
  - /tutorials/agent-platform/multi-cluster-access/
  - /tutorials/ai-agents/multi-cluster-access/
---
...
Register each cluster's `mcp-kubernetes` as a remote [`MCPServer`]({{< relref "/agent-platform/tutorials/managing-mcp-servers" >}}) and put them all in one **family** so their tools appear once, with an argument that selects the target cluster:
...
Where a remote server trusts the **same** issuer as the central Muster, you don't need exchange at all—set `forwardToken: true` instead and Muster reuses the user's token. See [RBAC and SSO]({{< relref "/agent-platform/tutorials/access-control" >}}) for the difference.
...
- **Large tokens overflow default ingress buffers.** Enterprise tokens carrying many group claims can exceed the default nginx header buffer. The `mcp-kubernetes` ingress needs a raised `large_client_header_buffers` to accept them. See [RBAC and SSO]({{< relref "/agent-platform/tutorials/access-control" >}}#large-tokens) for the symptom and fix.

## Related

- [Manage MCP servers]({{< relref "/agent-platform/tutorials/managing-mcp-servers" >}}): the `family` and `auth` fields in full.
- [Map RBAC and SSO]({{< relref "/agent-platform/tutorials/access-control" >}}): mapping identity to cluster permissions.
- [Architecture]({{< relref "/agent-platform/overview/architecture" >}}): how fleet-wide aggregation fits together.
