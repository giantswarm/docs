...
menu:
  principal:
    ...
    identifier: agent-platform-tutorials-toolset-presets
owner:
...
An [installation on your own cluster]({{< relref "/agent-platform/tutorials/install" >}}) gets both presets with the same chart, which also pins the Muster floor they need. Don't add them by hand on an older platform release: a Muster older than 5.12.0 refuses to start on a preset with a `label:` rule.
...
| `server: <name>` | Every tool of that `MCPServer` resource—the family name or a member's name for a [family]({{< relref "/agent-platform/tutorials/managing-mcp-servers" >}}#group-equivalent-servers-with-family) |
...
Commit the values through your GitOps pipeline. The configuration change rolls the Muster pod once. Then, with an [authenticated CLI context]({{< relref "/agent-platform/reference/muster/cli/auth" >}}), ask Muster what a preset resolves to for you:
...
- [Toolsets reference]({{< relref "/agent-platform/reference/muster/toolsets" >}}): selector grammar, rule shapes, built-in and platform presets, the chart value, and agent-manager's argument.
- [Manage MCP servers]({{< relref "/agent-platform/tutorials/managing-mcp-servers" >}}): the `MCPServer` resources and labels the presets select over.
- [Create and deploy an agent]({{< relref "/agent-platform/tutorials/create-an-agent" >}}): where agent authors pick from the presets you define.
