...
Any MCP server can sit behind Muster—internal tools, vendor services, the Model Context Protocol project's reference servers, or a commercial remote server. You register it the same way as any other server, with an [`MCPServer` resource]({{< relref "/agent-platform/tutorials/managing-mcp-servers" >}}), and it shows up in the developer portal under **Registered servers**, next to the platform's own Infrastructure and Agent Platform servers. This guide covers the parts specific to third-party servers: unauthenticated servers, bearer-token servers, and OAuth servers that don't advertise their authorization server the standard way.
