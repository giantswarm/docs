...
menu:
  principal:
    ...
    identifier: agent-platform-tutorials-enable-portal-features
owner:
...
aliases:
  - /tutorials/agent-platform/enable-portal-features/
---

The [Agent Platform section]({{< relref "/agent-platform/overview/developer-portal" >}}) ships with the Giant Swarm Backstage plugins but is off by default. Enabling it takes several independent configuration changes—none of them discoverable from the portal itself. This guide walks them in order: first the browse experience, then the extra pieces deploying agents needs.
...
- [Agent Platform in the developer portal]({{< relref "/agent-platform/overview/developer-portal" >}}) - What you just enabled.
- [Create and deploy an agent]({{< relref "/agent-platform/tutorials/create-an-agent" >}}) - The flow these settings unlock.
- [Install the Agent Platform on your own cluster]({{< relref "/agent-platform/tutorials/install" >}}) - Where the platform side of this configuration lives.
