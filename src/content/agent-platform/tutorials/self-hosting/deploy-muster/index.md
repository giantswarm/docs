...
menu:
  principal:
    ...
    identifier: agent-platform-tutorials-self-hosting-deploy-muster
owner:
...
aliases:
  - /tutorials/agent-platform/self-hosting/deploy-muster/
  - /tutorials/ai-agents/self-hosting/deploy-muster/
---
...
For the concepts behind the aggregator, see the [architecture overview]({{< relref "/agent-platform/overview/architecture" >}}). To protect the deployed endpoint with single sign-on, continue to [set up OAuth]({{< relref "/agent-platform/tutorials/self-hosting/oauth-setup" >}}).
...
- An OIDC provider (Dex on a Giant Swarm cluster) if you plan to protect the endpoint, covered in [OAuth setup]({{< relref "/agent-platform/tutorials/self-hosting/oauth-setup" >}}).
...
With `kubernetes: true`, the aggregator watches the cluster for `MCPServer` and `Workflow` resources in `namespace` and reconciles them live. You add a downstream server by applying an [`MCPServer`]({{< relref "/agent-platform/tutorials/managing-mcp-servers" >}}) resource, and a workflow by applying a [`Workflow`]({{< relref "/agent-platform/tutorials/authoring-workflows" >}}). No restart is needed. `namespace` defaults to the release namespace, so this configuration watches the `muster` namespace.
...
Tokens from enterprise single sign-on can carry many group claims and overflow the default ingress header buffers. Raise `large_client_header_buffers` on the ingress that fronts Muster. The [OAuth setup]({{< relref "/agent-platform/tutorials/self-hosting/oauth-setup" >}}) guide covers the symptom and fix.
...
- [Set up OAuth]({{< relref "/agent-platform/tutorials/self-hosting/oauth-setup" >}}): protect the endpoint and proxy authentication to downstream servers.
- [Manage MCP servers]({{< relref "/agent-platform/tutorials/managing-mcp-servers" >}}): register the downstream servers Muster aggregates.
- [Multi-cluster token exchange]({{< relref "/agent-platform/tutorials/self-hosting/multi-mc-token-exchange" >}}): bridge single sign-on to remote management clusters.
