...
menu:
  principal:
    ...
    identifier: agent-platform-tutorials-self-hosting
owner:
...
- [Deploy Muster]({{< relref "/agent-platform/tutorials/self-hosting/deploy-muster" >}}): install the CRD and application Helm charts and run the aggregator in custom-resource discovery mode.
- [Set up OAuth]({{< relref "/agent-platform/tutorials/self-hosting/oauth-setup" >}}): protect the endpoint with Dex and configure the proxy that authenticates to downstream servers.
- [Multi-cluster token exchange]({{< relref "/agent-platform/tutorials/self-hosting/multi-mc-token-exchange" >}}): bridge single sign-on to remote management clusters with RFC 8693.
