...
menu:
  principal:
    ...
    identifier: agent-platform-tutorials-self-hosting-oauth-setup
owner:
...
aliases:
  - /tutorials/agent-platform/self-hosting/oauth-setup/
  - /tutorials/ai-agents/self-hosting/oauth-setup/
---
...
This guide assumes Muster is already [deployed]({{< relref "/agent-platform/tutorials/self-hosting/deploy-muster" >}}). For the security model behind it, see the [security overview]({{< relref "/agent-platform/overview/security" >}}).
...
How Muster reuses or exchanges the user's token per downstream server is set on each [`MCPServer`]({{< relref "/agent-platform/tutorials/managing-mcp-servers" >}}#configure-authentication) resource, not here.
...
- [Deploy Muster]({{< relref "/agent-platform/tutorials/self-hosting/deploy-muster" >}}): install the charts this guide configures.
- [Map RBAC and SSO]({{< relref "/agent-platform/tutorials/access-control" >}}): connect identity-provider groups to cluster permissions.
- [Multi-cluster token exchange]({{< relref "/agent-platform/tutorials/self-hosting/multi-mc-token-exchange" >}}): cross-cluster single sign-on with RFC 8693.
- [Security model]({{< relref "/agent-platform/overview/security" >}}): per-user visibility and token handling.
