...
menu:
  principal:
    ...
    identifier: agent-platform-overview-toolsets
owner:
...
aliases:
  - /overview/agent-platform/toolsets/
---
...
- **Tool group**: the platform's grouping of MCP servers into *Infrastructure*, *Agent Platform*, and *Registered servers*, plus *Workflows* on the Tools step. See the [three groups]({{< relref "/agent-platform/overview/introduction" >}}#three-groups-of-mcp-servers) in the introduction.
...
How to define presets for your installation is covered in [Define toolset presets]({{< relref "/agent-platform/tutorials/toolset-presets" >}}).
...
- **Creating an agent in the portal.** The creation wizard has a **Tools** step that opens with nothing selected—which means no tools—and leads with the presets; see [Create and deploy an agent]({{< relref "/agent-platform/tutorials/create-an-agent" >}}#step-3-compose-the-toolset).
- **Reviewing an agent.** The agent page's **Toolset** card shows the declaration, what it resolves to for you, and the *Implicit full access*, *No tools*, and *Full gateway access* labels; see the [portal section]({{< relref "/agent-platform/overview/developer-portal" >}}#what-an-agent-can-use).
...
- **From any MCP client.** The header is plain HTTP: a client that lets you set request headers can send a toolset too. See [Define toolset presets]({{< relref "/agent-platform/tutorials/toolset-presets" >}}#send-a-toolset-from-your-own-mcp-client).

The complete grammar, the preset rule shapes, the error texts, and the chart and agent-manager contracts are in the [toolsets reference]({{< relref "/agent-platform/reference/muster/toolsets" >}}).
