...
menu:
  principal:
    ...
    identifier: agent-platform-overview-developer-portal
owner:
...
aliases:
  - /overview/developer-portal/agent-platform/
---

The developer portal's **Agent Platform** section is the visual home of the [Agent Platform]({{< relref "/agent-platform/overview/introduction" >}}): browse the MCP servers behind the gateway, explore and run their tools, inspect workflows, create and manage agents, and review your chat sessions. Everything it shows is the live state of the platform's Kubernetes resources—the portal is a peer client of the same declarative API as `kubectl` and GitOps.

To turn the section on in your portal instance, see [Enable Agent Platform features in Backstage]({{< relref "/agent-platform/tutorials/enable-portal-features" >}}).
...
- **Workflows.** The platform's [workflows]({{< relref "/agent-platform/tutorials/authoring-workflows" >}}) with their steps, validity, execution statistics, and a run button with execution history.
...
- **Agents.** The agents running on the platform, each with its readiness, configuration, system prompt, skills, toolset, and owning deployment—plus the [create-an-agent flow]({{< relref "/agent-platform/tutorials/create-an-agent" >}}), whose required **Tools** step composes the toolset.
...
- **Models.** The model configurations agents can run on, per installation, each with the accepted state the runtime reports and the reason when it isn't, plus a form to add one with your own API key. Nothing is accepted until you [bring your own model]({{< relref "/agent-platform/tutorials/bring-your-own-model" >}}).

## Three groups of MCP servers

The servers page sorts MCP servers the way the whole platform does, and the names match the [introduction]({{< relref "/agent-platform/overview/introduction" >}}#three-groups-of-mcp-servers):
