...
menu:
  principal:
    ...
    identifier: agent-platform-overview-architecture
owner:
...
aliases:
  - /overview/agent-platform/architecture/
...
The [platform integration]({{< relref "/agent-platform/overview/platform-integration" >}}) page covers what that means for RBAC and auditing; the [security]({{< relref "/agent-platform/overview/security" >}}) page covers the token handling.
...
The trade-off is scope: a workflow only checks what it was authored to check. For a first responder handling the specific alert that paged, that focus is a feature. For open-ended "what's broken here?" exploration, an agent with the raw tools surfaces more adjacent context at higher cost. Both modes run through the same gateway—see [meta-tools]({{< relref "/agent-platform/overview/meta-tools" >}}) for how an agent reaches each one.
