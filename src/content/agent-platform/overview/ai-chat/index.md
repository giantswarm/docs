...
menu:
  principal:
    ...
    identifier: agent-platform-overview-ai-chat
owner:
...
aliases:
  - /overview/developer-portal/ai-chat/
---

The developer portal includes a built-in AI chat. You ask a question in plain language and get an answer grounded in live platform state, without leaving the portal or switching to a terminal. It's the same assistant experience you'd configure in your [IDE]({{< relref "/agent-platform/getting-started/ai-agent-setup" >}}), but already wired up and waiting in the browser.

Behind the chat is **Muster**, the platform's MCP gateway. The portal connects to your central Muster instance, so the assistant can reach your whole fleet through one secure, authenticated endpoint. For the concepts behind that gateway, see [AI agents on the platform]({{< relref "/agent-platform/overview" >}}).
...
The [AI agent security page]({{< relref "/agent-platform/overview/security" >}}) explains the OAuth flow, per-user tool visibility, and single sign-on across clusters.
...
To use the same assistant from your editor instead of the browser, follow [Set up your AI agent]({{< relref "/agent-platform/getting-started/ai-agent-setup" >}}).
