...
menu:
  principal:
    ...
    identifier: agent-platform-overview-platform-integration
owner:
...
aliases:
  - /overview/agent-platform/platform-integration/
---
...
The platform uses **one OIDC identity provider per installation**. On Giant Swarm clusters that's [Dex]({{< relref "/container-platform/overview/architecture/authentication" >}}), federating to your upstream provider (GitHub, Entra ID, and others)—but the contract is plain OIDC, so any compliant provider works. On a self-hosted install you point the chart's identity configuration at your own issuer.
...
The important property: the platform never mints its own identity tokens. Your identity provider is the sole authority—Muster signs nothing, and everything downstream validates tokens against the provider that issued them. The [security]({{< relref "/agent-platform/overview/security" >}}) and [authentication]({{< relref "/agent-platform/overview/authentication" >}}) pages cover the token mechanics in depth.
...
- [Agent Platform architecture]({{< relref "/agent-platform/overview/architecture" >}}) - The request path these integrations attach to.
- [Security]({{< relref "/agent-platform/overview/security" >}}) - The OAuth model, per-user tool visibility, and sign-out.
- [Authentication deep dive]({{< relref "/agent-platform/overview/authentication" >}}) - Every token on the path, end to end.
- [Observability overview]({{< relref "/observability-platform/overview" >}}) - The platform's monitoring stack these components plug into.
