...
menu:
  principal:
    identifier: agent-platform-reference-muster-cli-serve
...
To connect an IDE that can't reach the endpoint directly, bridge it with [`muster agent --mcp-server`]({{< relref "/agent-platform/reference/muster/cli/agent" >}}). IDEs that support remote, OAuth-protected MCP connect straight to the aggregator URL instead.
...
These flags turn Muster into an OAuth 2.1 protected resource and enable the MCP client proxy for authenticating to remote MCP servers. Full setup needs a configuration file. See [Set up OAuth for Muster]({{< relref "/agent-platform/tutorials/self-hosting/oauth-setup" >}}).
...
- [`muster standalone`]({{< relref "/agent-platform/reference/muster/cli/standalone" >}}) - Run the aggregator and agent in one process.
- [`muster agent`]({{< relref "/agent-platform/reference/muster/cli/agent" >}}) - Connect to a running aggregator.
- [Muster architecture]({{< relref "/agent-platform/overview/architecture" >}}) - How the aggregator works.
