...
menu:
  principal:
    identifier: agent-platform-reference-muster-cli-call
...
Any other `--key=value` flag is forwarded to the tool as an argument. `call` also accepts the [common flags]({{< relref "/agent-platform/reference/muster" >}}#common-flags).
...
- [Meta-tools]({{< relref "/agent-platform/reference/muster/meta-tools" >}}) - The tool families and the `core_*` catalog.
- [`muster start`]({{< relref "/agent-platform/reference/muster/cli/start" >}}) - Run a workflow by name.
