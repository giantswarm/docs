...
As an authenticated user you can upgrade the cluster to the **next active workload cluster release**, using the web UI or the CLI. The web UI shows a yellow link next to the version information if there is an upgrade available. For the CLI you can use the command [`kubectl-gs update cluster`]({{< relref "/reference/kubectl-gs/update-cluster" >}})) for the same purpose.
