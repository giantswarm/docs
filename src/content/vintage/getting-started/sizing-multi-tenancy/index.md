...
2. What kind of workloads will be running in there and how many resources do they need?

In general, a cluster that will only be short-lived and for testing purposes can be relatively small and doesn't need much buffer.
Otherwise, you should always account for at least 1 node buffer, so in case one of your nodes goes down or gets rolled due to updates your workloads still have room to be scheduled.

Furthermore, once you want to actually use the cluster productively, we would recommend having at least 4 worker nodes in your cluster.
...
To determine the right sizing in terms of cores and RAM, you need to know what kind of workloads will be run on the cluster
 and how many resources they need.
...
While you might have a variable amount of short-lived clusters (e.g. for testing or playgrounds), you also want to have a specific amount of clusters that are fixed.
Typically, there would be 1 cluster per stage. Example:
