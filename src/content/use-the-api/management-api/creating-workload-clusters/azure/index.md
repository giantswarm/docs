...
Starting from version {{% first_azure_nodepools_version %}} on Azure, Giant Swarm introduced a feature to create multiple [node pools]({{< relref "/advanced/cluster-management/node-pools-vintage" >}}) on Azure.
...
- [MachinePool]({{< relref "/use-the-api/management-api/crd/machinepools.exp.cluster.x-k8s.io.md" >}}) - represents a node pool.

The CRs above then reference provider specific implementations. In our case, for clusters on Azure, they are:

- [AzureCluster]({{< relref "/use-the-api/management-api/crd/azureclusters.infrastructure.cluster.x-k8s.io.md" >}}) - represents a workload cluster.
- [AzureMachinePool]({{< relref "/use-the-api/management-api/crd/azuremachinepools.exp.infrastructure.cluster.x-k8s.io.md" >}}) - configures the Azure-specific details of worker nodes in a node pool.
