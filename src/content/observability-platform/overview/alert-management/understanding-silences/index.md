...
menu:
  principal:
    ...
    identifier: observability-platform-overview-alert-management-understanding-silences
...
aliases:
  - /overview/observability/alert-management/understanding-silences/
---

This page explains what alert silences are and how they behave on the Giant Swarm Observability Platform, so you can decide when and how to use them. For the step-by-step mechanics of creating and managing silences, see the [silences how-to]({{< relref "/observability-platform/overview/alert-management/silences" >}}).
...
Both approaches integrate with the platform's [multi-tenancy model]({{< relref "/observability-platform/overview/configuration/multi-tenancy" >}}): a silence affects only alerts within its own tenant, and it must carry a tenant label referencing an existing Grafana Organization. Silences referencing a non-existent tenant are ignored.

## Where silences sit in the alerting pipeline

Silences act at the Alertmanager level, after alert rules are evaluated but before [alert routing]({{< relref "/observability-platform/overview/alert-management/alert-routing" >}}) delivers notifications. Three consequences follow from that position:
