...
menu:
  principal:
    parent: observability-platform-overview-data-management-data-ingestion
    identifier: observability-platform-overview-data-management-data-ingestion-servicemonitor-reference
...
aliases:
  - /overview/observability/data-management/data-ingestion/servicemonitor-reference/
---
...
For the task-oriented steps to set up metrics collection, see the [data ingestion guide]({{< relref "/observability-platform/overview/data-management/data-ingestion" >}}). For the complete upstream schema, see the [Prometheus Operator API documentation](https://github.com/prometheus-operator/prometheus-operator/blob/main/Documentation/api-reference/api.md).

## Discovery requirements

For a `ServiceMonitor` or `PodMonitor` to be picked up by the platform:

- **Tenant label**: the resource must carry the `observability.giantswarm.io/tenant` label. It routes the collected metrics to that [tenant]({{< relref "/observability-platform/overview/configuration/multi-tenancy" >}}).
- **Tenant existence**: the named tenant must already exist in a [Grafana Organization]({{< relref "/observability-platform/overview/configuration/creating-grafana-organization" >}}). Metrics routed to a non-existent tenant are dropped.
...
- [Data ingestion]({{< relref "/observability-platform/overview/data-management/data-ingestion" >}}): how to configure metrics collection, with working ServiceMonitor and PodMonitor examples
