...
menu:
  principal:
    parent: observability-platform-overview-data-management-data-import-export
    identifier: observability-platform-overview-data-management-data-import-export-import
...
aliases:
  - /overview/observability/data-management/data-import-export/import/
---

This guide shows you how to send metrics, logs, and traces from external sources into the Giant Swarm observability platform. Use it when the data originates outside a Giant Swarm managed cluster, for example from a SaaS application, an external database, or another cloud environment. For the concepts behind import, see [data import and export]({{< relref "/observability-platform/overview/data-management/data-import-export" >}}). To send data from inside a managed cluster, use [data ingestion]({{< relref "/observability-platform/overview/data-management/data-ingestion" >}}) instead.
...
2. **A tenant**: create or identify the tenant where your external data should be stored. The tenant must exist in a [Grafana Organization]({{< relref "/observability-platform/overview/configuration/multi-tenancy/" >}}).
...
The platform accepts data through standard observability APIs. For the full list of endpoints and the authentication headers they expect, see the [Observability Platform API reference]({{< relref "/observability-platform/overview/data-management/data-import-export/observability-platform-api" >}}).
...
The [Observability Platform API reference]({{< relref "/observability-platform/overview/data-management/data-import-export/observability-platform-api#oidc-provider-examples" >}}) lists the `token_url` and `scopes` values for common OIDC providers such as Azure AD, Google, Okta, and Keycloak.
...
4. **Set up tenants**: create the [Grafana Organizations]({{< relref "/observability-platform/overview/configuration/multi-tenancy/" >}}) for your external data.
...
- **[Export data]({{< relref "/observability-platform/overview/data-management/data-import-export/export" >}})**: read platform data from external tools.
...
- **[Multi-tenancy]({{< relref "/observability-platform/overview/configuration/multi-tenancy/" >}})**: set up the tenants your imported data routes to.
