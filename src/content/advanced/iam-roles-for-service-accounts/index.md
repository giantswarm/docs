...
Make sure the resource has the `alpha.aws.giantswarm.io/iam-roles-for-service-accounts` annotation **only right before applying a new cluster or just before upgrading to the latest AWS release. This is important to remember as the lingering annotation without the activation can cause troubles external-dns that also uses IRSA**. The value can be anything you like, as only the presence of that annotation is checked. Here is an example:
