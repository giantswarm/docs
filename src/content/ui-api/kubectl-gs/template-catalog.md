...
The `template catalog` command allows to create an [app catalog]({{< relref "/developer-platform/app-platform" >}}) manifest. The resulting manifest is meant to be applied to the management cluster, for example via `kubectl apply`.
...
The Catalog CRD supports having a related `ConfigMap` and/or `Secret` resource with values YAML. These values are merged with the rest of the [configuration]({{< relref "/developer-platform/app-platform/app-configuration/index.md" >}}) when Apps are deployed from this App Catalog.
