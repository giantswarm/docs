...
menu:
  main:
    identifier: reference-platform-api-authentication-automation
    parent: reference-platform-api-authentication
...
**Note:** In the context of this article we are assuming that you want to connect to the Platform API from outside the management cluster. In case your automation is running in the management cluster, using a service for authentication becomes much simpler.
...
Make sure that you have an authenticated `kubectl` context for your Platform API. We provide extensive documentation on [how to authenticate as a user]({{< relref "/vintage/use-the-api/management-api/authentication/user" >}}).
...
Here, let `FILE` be the path of a file that does not yet exist, e. g. `kubeconfig.yaml`. `API_ENDPOINT` is the Platform API endpoint from step 4. `CA_CERTIFICATE_PATH` is the path of the CA file created in step 3.
...
- [Authorization in the Platform API]({{< relref "/vintage/use-the-api/management-api/authorization" >}}) explains how to assign permissions to authenticated users
