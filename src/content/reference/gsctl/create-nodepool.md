...
- `--nodes-min`: Minimum number of worker nodes for the node pool. Defaults to `3` on both AWS and Azure.
- `--nodes-max`: Maximum number of worker nodes for the node pool. Defaults to `10` on AWS, and `3` on Azure.
