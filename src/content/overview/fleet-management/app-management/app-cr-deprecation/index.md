...
The Giant Swarm App custom resource (`App`, in the `application.giantswarm.io` API group) is being phased out in favor of upstream [Flux HelmRelease](https://fluxcd.io/flux/components/helm/helmreleases/). Every Giant Swarm management cluster already runs Flux — we're now making it the recommended way for customers to deploy and configure applications.
...
- **More expressive feature set.** HelmRelease offers dependency ordering (`dependsOn`), drift detection, post-renderers, fine-grained install/upgrade/rollback policies, and `valuesFrom` with explicit merge order — features that map naturally onto things we've layered on top of App CRs over the years.
...
- **Existing App CR deployments keep working.** There will be a long, supported overlap period — don't rush a migration.
