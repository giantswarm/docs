...
What an agent can *reach* with your token is one question; what it's *composed with* is another. Every agent created through the portal or agent-manager declares a [toolset]({{< relref "/overview/agent-platform/toolsets" >}}), which Muster applies on every request as a filter on the catalog the agent's meta-tools can see and call—always intersected with your own access, never widening it. A toolset is composition, not authorization: your identity and each backend's own authorization remain the boundary, and an agent without a toolset has *implicit full access* to whatever you can reach.
