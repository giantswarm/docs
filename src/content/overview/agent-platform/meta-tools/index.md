...
- **`workflow_<name>`**: a dynamically generated tool for a [workflow]({{< relref "/overview/agent-platform/architecture" >}}#workflows-cut-agent-token-cost). Authoring a workflow registers a matching `workflow_<name>` tool that the agent can discover and call like any other.
